...
for window_size in [int(20),int(50),int(100)]:
    for e in epochs_:
        ...
        pickle.dump(results,open(os.path.join(working_dir,
                                              'similarity measure result_sub%s_load%s_day%s_%dHz.p'%(sub,load,day,1000/window_size)),'wb')  )  
        ...
        ax.set(xlabel='Delay',ylabel='Encode',title='Euclidean distance\nsub%s_load%s_day%s\nsignal at %d Hz'%(sub,load,day,1000/window_size))
        ax.axhline(2000,color='k',lw=2,)
        plt.colorbar(im)
        fig.savefig(os.path.join(saving_dir,'Euclidean distance_sub%s_load%s_day%s_%dHz.png'%(sub,load,day,1000/window_size)),dpi=300)
        ...
        ax.axhline(2000,color='k',lw=2,)
        plt.colorbar(im)
        fig.savefig(os.path.join(saving_dir,'Cosine distance_sub%s_load%s_day%s_%dHz.png'%(sub,load,day,1000/window_size)),dpi=300)
        plt.close('all')
